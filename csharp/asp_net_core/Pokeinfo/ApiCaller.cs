        ...
        public static async Task GetPokemonDataAsync(int PokeId, Action<pokemon> Callback)
        {
            ...
            using (var Client = new HttpClient())
            {
                try
                {
                    ...
                    JObject Poke = JsonConvert.DeserializeObject<JObject>(StringResponse);
                    pokemon pokemonData = JsonConvert.DeserializeObject < pokemon > (StringResponse);
                    List<string> Types = new List<string>();
                    foreach(var entry in Poke["types"])
                    {   
                        Types.Add(entry["type"]["name"].ToString());
                    }
                    pokemonData.ptypes = Types;
                    pokemonData.image = Poke["sprites"]["front_default"].ToString();
