...
export class HttpService {
  ...
  getPokemon(){
    let tempObservable = this._http.get('https://pokeapi.co/api/v2/pokemon/150/');
    tempObservable.subscribe(data => {console.log("Got the Pokemon!", data)
      for(var ability of data['abilities']){
        ...
        abilityCount.subscribe(data => console.log(data['pokemon'].length + " have the ability " + data['name'] + "!"));
      }
    });
  }
}
